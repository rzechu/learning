...
public class ProviderStateMiddleware
{
    ...
    /// <param name="paymentRepository">Payments repository for actioning provider state requests</param>
    public ProviderStateMiddleware(RequestDelegate next, IPaymentRepository paymentRepository)
    {
        _next = next;
        ...
        {
            ["an payment with ID {id} exists"] = EnsureEventExistsAsync
        };
    }
    ...
    private async Task EnsureEventExistsAsync(IDictionary<string, object> parameters)
    {
        ...
        await _paymentRepository.InsertAsync(new PaymentDto(id.GetInt32(), 15, "USD", "CreditCard", "12345"));
    }
